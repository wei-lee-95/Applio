...
def generate_config(sample_rate: int, model_path: str):
    config_dir = os.path.dirname(os.path.abspath(__file__))  # .../rvc/train/extract
    rvc_root = os.path.abspath(os.path.join(config_dir, "../../"))  # 回到 rvc 根目錄

    config_path = os.path.join(rvc_root, "configs", f"{sample_rate}.json")  # 加上這行
    config_save_path = os.path.join(model_path, "config.json")  # 這邊通常是 model_path 下
